...
def plot_shap_summary(X, clf, output_path: str = "shap_summary.png"):
    ...
    Retrieves feature names directly from ColumnTransformer via get_feature_names_out().
    Disables additivity checking during shap_values call to avoid errors.
    """
    if X.shape[0] == 0:
    ...
    # Transform the sample (ensure it's dense)
    X_transformed = prep.transform(sample)
    if hasattr(X_transformed, "toarray"):
        X_transformed = X_transformed.toarray()

    # Let the ColumnTransformer tell us the output names
    ...
    # Plot summary using the exact feature names
    shap.summary_plot(
    ...
    """
    Generate and save a SHAP summary plot on a sample of X, saving to 'images/'.
    Retrieves feature names directly from ColumnTransformer by supplying the original column names.
    Disables additivity checking during shap_values call.
    """
    if X.shape[0] == 0:
        print("No samples for SHAP plot.")
        return
    ...
    # Get output feature names directly from the preprocessor by providing original input names
    # X.columns must match the order used in training:
    original_feature_names = list(sample.columns)
    feature_names = prep.get_feature_names_out(original_feature_names)

    # Create SHAP explainer
    explainer = shap.TreeExplainer(model)
    shap_values = explainer.shap_values(X_transformed, check_additivity=False)

    # Plot summary, using the exact feature_names
    shap.summary_plot(
        shap_values[1],            # SHAP values for positive class
        X_transformed,
        feature_names=feature_names,
    ...
    """
    Generate and save a SHAP summary plot on a sample of X, saving to 'images/'.
    We retrieve feature names directly from the ColumnTransformer to match X_transformed.
    """
    if X.shape[0] == 0:
        print("No samples for SHAP plot.")
        return
    ...
    # Get output feature names directly from the preprocessor
    # (sklearn ≥1.0)
    try:
        feature_names = prep.get_feature_names_out()
    except AttributeError:
        # Older sklearn: fall back to concatenating manually
        numeric_cols = [
            "air_temp_K", "proc_temp_K", "rot_speed_rpm", "torque_Nm", "tool_wear_min",
            "setup_time", "processing_time"
        ]
    ...
    # Create SHAP explainer (no additivity check argument needed here)
    explainer = shap.TreeExplainer(model)
    shap_values = explainer.shap_values(X_transformed, check_additivity=False)

    # Plot summary, using precomputed feature_names
    shap.summary_plot(
